...
class ClassController {
  ...
  static async getAllClasses(req, res) {
    try {
      ...
      if (!ClassModel || !ClassModel.getAllClasses) {
        ...
        return res
          .status(500)
          .json({ error: "Método getAllClasses não encontrado" });
      }

      const classes = await ClassModel.getAllClasses();

      console.log(
        "🔍 [DEBUG] Aulas encontradas:",
  ...
  static async getClassesByModuleId(req, res) {
    try {
      ...
      if (!ClassModel || !ClassModel.getClassesByModuleId) {
        console.error(
          "❌ [DEBUG] ClassModel.getClassesByModuleId não encontrado"
        );
        return res
          .status(500)
          .json({ error: "Método de busca de aulas não encontrado" });
      }
  ...
  static async createClass(req, res) {
    try {
      const { name, description, url_video, id_module, class_order } = req.body;

      // Validações básicas
      if (!name || !module_id) {
        return res
          .status(400)
          .json({ error: "Nome e ID do módulo são obrigatórios." });
      }

      // Verificar se o módulo existe
      const module = await ModuleModel.getModuleById(id_module);
      if (!module) {
        return res.status(404).json({ error: "Módulo não encontrado" });
      }
  ...
  static async updateClass(req, res) {
    try {
      const { id } = req.params;
      const { name, description, url_video, class_order } = req.body;
  ...
  static async getClassesWithModuleInfo(req, res) {
    try {
      ...
      console.log(
        "🔍 [DEBUG] Aulas com info encontradas:",
        classes ? classes.length : 0
      );
      return res.status(200).json(classes || []);
    } catch (error) {
      ...
      return res
        .status(500)
        .json({ error: "Erro ao listar aulas com informações do módulo." });
    }
  }
}
