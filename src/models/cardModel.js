...
class CardModel {
  static async getAllCards() {
    try {
      const result = await db.query('SELECT * FROM card ORDER BY created_at DESC');
      return result.rows;
    } catch (error) {
  ...
  static async getCardByTitle(title) {
    try {
      const result = await db.query('SELECT * FROM card WHERE title ILIKE $1', [`%${title}%`]);
      return result.rows;
    } catch (error) {
      console.error('Erro ao buscar card:', error);
      throw error;
  ...
  static async favoriteCard(userId, cardId) {
    try {
      ...
      await db.query(
        'UPDATE card SET fav = true WHERE id = $1',
        [cardId]
      );
  ...
  static async unfavoriteCard(userId, cardId) {
    try {
      ...
      if (parseInt(otherUsers.rows[0].count) === 0) {
        await db.query(
          'UPDATE card SET fav = false WHERE id = $1',
          [cardId]
        );
      }
